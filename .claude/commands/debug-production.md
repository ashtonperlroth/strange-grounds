...
1. curl the tRPC health endpoint:
   ```bash
   curl -s -o /dev/null -w "%{http_code}" https://strange-grounds.vercel.app/api/trpc
   ```
2. curl the Inngest serve endpoint:
   ```bash
   curl -s -o /dev/null -w "%{http_code}" https://strange-grounds.vercel.app/api/inngest
   ```
...
7. If VERCEL_TOKEN is available:
   ```bash
   npx vercel logs https://strange-grounds.vercel.app --token $VERCEL_TOKEN --limit 50 2>&1 | grep -i "error\|500\|404\|timeout"
   ```
...
11. Run the debug network spec:
    ```bash
    PLAYWRIGHT_BASE_URL=https://strange-grounds.vercel.app npx playwright test tests/debug-network.spec.ts --reporter=list
    ```
